...
export const useDeletePurchase = () => {
    const { modal, deletePurchase, purchases, resetDeletePurchaseState, resetModal } = usePurchaseStore();
    ...
    useEffect(() => {
        if (deletePurchase.state.state === "success") {
            toast.success(deletePurchase.state.data?.message || "Purchase deleted successfully");
        }
