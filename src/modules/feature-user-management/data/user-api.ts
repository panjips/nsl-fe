...
export const userApi = {
  users: async (type?: string) => {
    try {
      const response = await axiosInstance.get<ApiResponse<UserWithRole[]>>(
        ENDPOINTS.USER,
        {
          params: {
            type: type,
  ...
  createUser: async (data: CreateCategoryDTOType) => {
    try {
      const response = await axiosInstance.post<ApiResponse<UserWithRole>>(
        ENDPOINTS.USER,
        data
      );
      return response.data;
  ...
  updateUser: async (id: string | number, data: CreateCategoryDTOType) => {
    try {
      const response = await axiosInstance.put<ApiResponse<UserWithRole>>(
        `${ENDPOINTS.USER}/${id}`,
        data
      );
      return response.data;
