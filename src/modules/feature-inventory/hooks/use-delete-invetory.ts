...
export const useDeleteInventory = () => {
    const { modal, deleteInventory, inventories, resetDeleteInventoryState, resetModal } = useInventoryStore();
    ...
    useEffect(() => {
        if (deleteInventory.state.state === "success") {
            toast.success(deleteInventory.state.data?.message || "Inventory item deleted successfully");
        }
