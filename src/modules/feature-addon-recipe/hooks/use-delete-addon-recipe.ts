...
export const useDeleteAddonRecipe = () => {
    const { modal, deleteAddonRecipe, addonRecipes, resetDeleteAddonRecipeState, resetModal } = useAddonRecipeStore();
    ...
    useEffect(() => {
        if (deleteAddonRecipe.state.state === "success") {
            toast.success(deleteAddonRecipe.state.data?.message || "Addon recipe deleted successfully");
        }
