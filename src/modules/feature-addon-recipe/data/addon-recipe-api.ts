...
export const addonRecipeApi = {
  getAddonRecipes: async () => {
    try {
      const response = await axiosInstance.get<ApiResponse<AddonRecipe[]>>(
        ENDPOINTS.ADDON_RECIPE
      );
  ...
  getAddonRecipeByAddonId: async (addonId: string | number) => {
    try {
      const response = await axiosInstance.get<ApiResponse<AddonRecipe>>(
        `${ENDPOINTS.ADDON_RECIPE}/${addonId}`
      );
      return response.data;
    } catch (error) {
      if (isAxiosError(error)) {
  ...
  updateAddonRecipe: async (
    id: string | number,
    data: BulkCreateAddonRecipeDTOType
  ) => {
    try {
      const response = await axiosInstance.put<ApiResponse<null>>(
        `${ENDPOINTS.ADDON_RECIPE}/${id}/bulk`,
        data
      );
      return response.data;
    } catch (error) {
