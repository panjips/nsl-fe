...
export const useDeleteProductRecipe = () => {
    const { modal, deleteProductRecipe, productRecipes, resetDeleteProductRecipeState, resetModal } =
    ...
    useEffect(() => {
        if (deleteProductRecipe.state.state === "success") {
            toast.success(deleteProductRecipe.state.data?.message || "Product recipe deleted successfully");
        }
